...
static void AddToS2(int buf_id){
	// Make sure no one in S2 is pointing to buf_id
	// if(StrategyControl->s2prev[buf_id] >= 0)
	//	Assert(StrategyControl->s2next[StrategyControl->s2prev[buf_id]] != buf_id);
	// if(StrategyControl->s2next[buf_id] >= 0)
	//	Assert(StrategyControl->s2prev[StrategyControl->s2next[buf_id]] != buf_id);
...
static void AddToS1(int buf_id){
	// Make sure no one in S1 is pointing to buf_id
	// if(StrategyControl->s1prev[buf_id])
	//	Assert(StrategyControl->s1next[StrategyControl->s1prev[buf_id]] != buf_id);
	// if(StrategyControl->s1next[buf_id])
	// 	Assert(StrategyControl->s1prev[StrategyControl->s1next[buf_id]] != buf_id);
...
static void AddToFreelist(volatile BufferDesc *buf){
	LockBufHdr(buf);
	elog(LOG, "Head of freelist: %d", StrategyControl->firstFreeBuffer);
	buf->freeNext = StrategyControl->firstFreeBuffer;
	StrategyControl->firstFreeBuffer = buf->buf_id;
	buf->usage_count = 0;
	elog(LOG, "New Head of freelist: %d", StrategyControl->firstFreeBuffer);
	UnlockBufHdr(buf);
};
...
StrategyUpdateAccessedBuffer(int buf_id, bool delete)
{
	...
	if(delete){
		if(StrategyControl->numAccess[buf_id] == 1){
			RemoveFromS1(buf_id);
			RemoveFromS2(buf_id);
		} else if(StrategyControl->numAccess[buf_id] > 1){
			RemoveFromS2(buf_id);
			RemoveFromS2(buf_id+NBuffers);
		}
		LWLockAcquire(BufFreelistLock, LW_EXCLUSIVE);
		AddToFreelist(&BufferDescriptors[buf_id]);
...
StrategyFreeBuffer(volatile BufferDesc *buf)
{
	...
	if (buf->freeNext == FREENEXT_NOT_IN_LIST)
	{
		...
		RemoveFromS2(buf->buf_id);
		if(StrategyControl->numAccess[buf->buf_id] == 1)
			RemoveFromS1(buf->buf_id);
		else if(StrategyControl->numAccess[buf->buf_id] > 1)
...
StrategyInitialize(bool init)
{
	...
	// elog(LOG, "End StrategyInitialize");
}
