...
void printS1Buffer() {
	...
	int lru = StrategyControl->s1lrubuf;
		int i = 0;
	while(1){

		str[i] = lru;
		i++;
//		elog(LOG, " logging: i:%d  lru:%d b", i, lru, lru == StrategyControl->s1mrubuf);
		lru = StrategyControl->s1prev[lru];
		if(lru == StrategyControl->s1mrubuf || i == 15){
			str[i] = lru;
			break;
		}
	}
	elog(LOG, "S1: mru: %d,  LRU: %d", StrategyControl->s1mrubuf, StrategyControl->s1lrubuf);
	elog(LOG, "S1: %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d", str[0], str[1], str[2],str[3], str[4], str[5], str[6], str[7], str[8], str[9],str[10],
		 str[11], str[12], str[13], str[14], str[15]);
};
void printS2Buffer() {
	int str[40];

	int lru = StrategyControl->s2lrubuf;
	int i = 0;
	while(1){

		str[i] = lru;
		i++;
//		elog(LOG, "logging: i:%d  lru:%d", i, lru);
		lru = StrategyControl->s2prev[lru];
		if(lru == StrategyControl->s2mrubuf || i == 31){
			str[i] = lru;
			break;
		}
	}
	elog(LOG, "S2: mru: %d,  LRU: %d", StrategyControl->s2mrubuf, StrategyControl->s2lrubuf);
	elog(LOG, "S2: %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d", str[0], str[1], str[2],str[3], str[4], str[5], str[6], str[7], str[8], str[9],str[10],
		 str[11], str[12], str[13], str[14], str[15], str[16], str[17], str[18], str[19], str[20], str[21], str[22], str[23], str[24], str[25], str[26], str[27], str[28], str[29], str[30], str[31]);
};
...
static void AddToS2(int buf_id){
	elog(LOG, "adding to S2: %d", buf_id);
//	printS2Buffer();
...
static void RemoveFromS2(int buf_id){
	elog(LOG, "removing from S2: %d", buf_id);
//	printS2Buffer();
...
static void AddToS1(int buf_id){
	elog(LOG, "adding to S1: %d", buf_id);
//	printS1Buffer();
...
static void RemoveFromS1(int buf_id){
	elog(LOG, "removing from S1: %d", buf_id);
//	printS1Buffer();
	// Link up both neighbours
...
{
	elog(LOG, "updateAccessed: %d", buf_id);
...
//		RemoveFromFreelist(buf);
	...
	elog(LOG, "nothing in freelist: %d", StrategyControl->firstFreeBuffer);
	...
	for(victim=StrategyControl->s1lrubuf; victim>=0; victim=StrategyControl->s1prev[victim]){
		...
		if(buf->refcount == 0){	/* Found a usable buffer */
			elog(LOG, "Using victim %d from S1. prev[victim]: %d", victim, StrategyControl->s1prev[victim]);
...
StrategyFreeBuffer(volatile BufferDesc *buf)
{
	...
	{
		 elog(LOG, "StrategyFreeBuffer: %d", buf->buf_id);
		/*
	 	* It is possible that we are told to put something in the freelist that
	 	* is already in it; don't screw up the list if so.
	 	*/
